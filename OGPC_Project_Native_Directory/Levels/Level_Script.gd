...
func set_player_spawnpoint_and_position_reality(health, player_position, spawnpoint, enemy1s, enemy2s, pixelated_bosses, tree):
	...
	# Reset your health just by clicking exit to menu. Crazy that I didn't let that happen.
...
func _process(delta):
	# Open the pause menu if that happens.
	if Input.is_action_just_pressed("ui_pause"):
		get_parent().Open_Pause_Menu()
	
	# Load the game if we haven't.
	if not loaded:
		loaded = true 
		# Reality because that older function that is gone now ):
